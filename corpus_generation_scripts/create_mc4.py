        ...
        with jsonlines.open(args.input_file) as reader:
            for mc4post in tqdm(reader):
                ...
                myarticle['id'] = args.doc_type+"_" + \
                    args.language_reported+"_"+slugify(mc4post['url'])
                ...
                myarticle['publish_date'] = datetime.strptime(
                    mc4post['timestamp'], '%Y-%m-%dT%H:%M:%SZ').strftime('%Y%m%d')
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Convert the MC4 to our json-format. This corpus is in ascii json format. Output is an UTF-8 file with one document per line")
