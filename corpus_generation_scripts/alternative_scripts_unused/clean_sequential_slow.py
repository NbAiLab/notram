...
def clean(line, config, pbar):
    pbar.update(len(line))
    data = load_jsonline([line])
    ...
    if config['replace_usernames_tweets']:
        data['text'] = data['text'].apply(lambda x: replace_usernames_tweets(x))
    ...
    if config['replace_urls']:
        data['text'] = data['text'].apply(lambda x: replace_urls(x))
    ...
    if config['replace_email_addresses']:
        data['text'] = data['text'].apply(lambda x: replace_email_addresses(x))
...
def main(args):
    ...
    with tqdm(total=os.path.getsize(args.input_file)) as pbar:
       with open(args.input_file) as f:
           Parallel(n_jobs=98, prefer="threads")(delayed(clean)(line, config, pbar) for line in f)
