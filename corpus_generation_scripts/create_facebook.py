####################################################################################
# Convert the WEB64 Facebook corpus to jsonl.
        ...
        for f in tqdm(filelist):
            with jsonlines.open(f) as reader:
                for facebookpost in reader:
                    ...
                    myarticle['id'] = args.doc_type+"_"+ str(facebookpost['post_id'])+"_"+ str(facebookpost['page_id']) 
                    myarticle['language_reported'] = args.language_reported
                    myarticle['paragraphs'] = []
                    myarticle['publish_date'] = datetime.strptime(facebookpost['created_time'], '%Y-%m-%dT%H:%M:%S+00:00').strftime('%Y%m%d')
